...
self.addEventListener('push', function (event) {
  ...
  var titulo = dados.title || "Barreto's Frota";
  var opcoes = {
    body: dados.body || '',
    icon: 'icon-192.png',
    badge: 'icon-192.png',
...
self.addEventListener('notificationclick', function (event) {
  ...
  var relativo = (event.notification.data && event.notification.data.url) || 'index.html';
  ...
  event.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function (lista) {
      for (var i = 0; i < lista.length; i++) {
        var cliente = lista[i];
        if (cliente.url.indexOf('index.html') !== -1 && 'focus' in cliente) {
